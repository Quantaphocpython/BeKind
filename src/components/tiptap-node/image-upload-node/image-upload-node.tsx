...
export const ImageUploadNode: React.FC<NodeViewProps> = (props) => {
  ...
  const handleUpload = async (files: File[]) => {
    ...
    if (urls.length > 0) {
      ...
      console.log('Insert position:', pos)

      if (isValidPosition(pos)) {
        // Create image nodes with proper structure
        const imageNodes = urls.map((url, index) => {
          const filename = files[index]?.name.replace(/\.[^/.]+$/, '') || 'unknown'
          console.log('Creating image node:', { url, filename })
          return {
            type: 'image',
            attrs: {
              src: url,
        ...
        // Insert images and remove the upload node
        props.editor
          .chain()
          .focus()
