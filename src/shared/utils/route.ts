...
export const replaceRouteParams = (path: string, params?: ParamConfig): string => {
  ...
  return Object.entries(params).reduce((acc, [key, value]) => acc.replace(`{${key}}`, value.toString()), path)
}
 ...
 */
export const routeConfig = (path: string, pathParams?: ParamConfig, queryParams?: QueryParams): string => {
